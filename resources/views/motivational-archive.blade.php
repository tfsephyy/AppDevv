...
<body>
                        ...
                        <tbody>
                            ...
                            <tr data-message-id="{{ $message->id }}">
                                ...
                                <td class="actions">
                                    ...
                                    <button class="btn-action unarchive" onclick="unarchiveMessage({{ $message->id }})">
                            ...
                            @empty
                            <tr>
                                <td colspan="3" class="no-data">No archived messages found.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        ...
        async function deleteMessage(id) {
            const confirmed = await showConfirmModal('Delete Message', 'Are you sure you want to permanently delete this message? This action cannot be undone.', 'Delete', true);
            if (confirmed) {
                try {
                    const response = await fetch(`/motivational/${id}`, {
                        method: 'DELETE',
        ...
        async function unarchiveMessage(id) {
            const confirmed = await showConfirmModal('Unarchive Message', 'Are you sure you want to unarchive this message?', 'Unarchive');
            if (confirmed) {
                try {
                    const response = await fetch(`/motivational/${id}/unarchive`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                        }
