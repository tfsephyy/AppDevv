...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Icon Action Buttons for Archive */
        .btn-action {
            width: 35px;
            height: 35px;
            border-radius: 6px;
        ...
        <div class="feed-content">
            ...
            <div class="content-header">
                <div class="content-tabs">
                    <button class="tab-btn active" data-tab="posts" onclick="switchFeedTab('posts')">Posts</button>
                    <button class="tab-btn" data-tab="archive" onclick="switchFeedTab('archive')">Archive</button>
                </div>
                ...
                <div class="post-card" data-post-id="{{ $post->id }}">
                    <div class="post-header">
                        ...
                        <div class="post-options">
                            <button class="options-btn" onclick="editPost({{ $post->id }})" title="Edit">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="options-btn" onclick="archivePost({{ $post->id }})" title="Archive">
            ...
            <form id="editPostForm" onsubmit="updatePost(event)">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="image-preview-container has-images" id="editCurrentImages">
                    ...
                    <div class="form-group">
                        ...
                        <div class="image-upload-area" onclick="document.getElementById('editImages').click()">
                            <i class="fas fa-images"></i>
                            <p>Click to upload new images</p>
                        </div>
                        <input type="file" id="editImages" name="images[]" multiple accept="image/*" style="display: none;" onchange="previewEditImages(event)">
                        <div class="image-preview-container" id="editNewImagesContainer">
    ...
    <script>
        ...
        function removeImage(index) {
            ...
            if (selectedFiles.length === 0) {
                document.querySelector('.image-preview-container').classList.remove('has-images');
            }
        }
        ...
        async function archivePost(postId) {
            ...
            const confirmed = await showConfirmModal('Archive Post', 'Are you sure you want to archive this post?', 'Archive');
            ...
            try {
                const response = await fetch(`/feed/${postId}/archive`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async function loadArchive() {
            try {
                const response = await fetch('{{ route("feed.archived") }}');
                ...
                archiveContent.innerHTML = posts.map(post => {
                    const displayImages = post.images ? post.images.slice(0, 3) : [];
                    const remainingCount = post.images ? post.images.length - 3 : 0;
                    const imageClass = !post.images || post.images.length === 0 ? '' : 
                    ...
                    <div class="post-card">
                            ...
                            <div class="post-images ${imageClass}">
                                ${displayImages.map((img, idx) => `
                                    <div class="post-image-wrapper">
                                        <img src="/storage/${img}" alt="Post image" class="post-image">
                                        ${idx === 2 && remainingCount > 0 ? `
                                            <div class="more-images-overlay">+${remainingCount}</div>
                                        ` : ''}
                                    </div>
                                `).join('')}
                            </div>
                        ` : ''}
                        <div class="post-actions">
                            <button class="btn-action unarchive" onclick="unarchivePost(${post.id})" title="Unarchive">
                                <i class="fas fa-box-open"></i>
                            </button>
                            <button class="btn-action delete" onclick="deletePost(${post.id})" title="Delete">
        ...
        async function unarchivePost(postId) {
            const confirmed = await showConfirmModal('Unarchive Post', 'Are you sure you want to unarchive this post?', 'Unarchive');
            if (!confirmed) return;

            try {
                const response = await fetch(`/feed/${postId}/unarchive`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async function deletePost(postId) {
            const confirmed = await showConfirmModal('Delete Post', 'Are you sure you want to delete this post permanently? This action cannot be undone.', 'Delete', true);
            if (!confirmed) return;
        ...
        async function editPost(postId) {
            try {
                ...
                // Get post images from the postImages object
                const images = postImages[postId] || [];
                ...
                // Display current images - extract path from full URL
                currentEditImages = images.map(img => {
                    // If it's a full URL, extract the path after domain
                    if (img.includes('http')) {
                        const url = new URL(img);
                        return url.pathname.replace('/storage/', '');
                    }
                    // Otherwise just remove /storage/ prefix if present
                ...
                document.getElementById('editNewImagesContainer').classList.remove('has-images');
        ...
        function displayCurrentImages() {
            ...
            const containerWrapper = document.getElementById('editCurrentImages');
            
            if (currentEditImages.length === 0) {
                containerWrapper.classList.remove('has-images');
                container.innerHTML = '<p style="text-align: center; color: var(--text-muted); padding: 20px;">No images</p>';
                return;
            }
            
            containerWrapper.classList.add('has-images');
            container.innerHTML = currentEditImages.map((img, index) => `
                <div class="preview-item">
                    <img src="/storage/${img}" class="preview-image">
        ...
        function previewEditImages(event) {
            const files = Array.from(event.target.files);
            const container = document.getElementById('editNewImagePreview');
            const containerWrapper = document.getElementById('editNewImagesContainer');
            ...
            files.forEach((file, index) => {
                ...
                reader.onload = function(e) {
                    ...
                    div.innerHTML = `
                        <img src="${e.target.result}" class="preview-image">
                        <button type="button" class="remove-image" onclick="removeNewEditImage(${index})">
                            <i class="fas fa-times"></i>
                        </button>
                    `;
        ...
        function removeNewEditImage(index) {
            ...
            previewEditImages({ target: input });
        }
        ...
        async function updatePost(event) {
            ...
            // Add new images
            const newImagesInput = document.getElementById('editImages');
